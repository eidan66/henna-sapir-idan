    ...
    for (let i = 0; i < validMediaItems.length; i++) {
      ...
      try {
        ...
        if (fileResponse.ok) {
          ...
          // Remove the henna-uploads/ prefix from the ID if it exists
          let cleanId = media.id;
          if (cleanId.startsWith('henna-uploads/')) {
            cleanId = cleanId.replace('henna-uploads/', '');
          }
...
שמחתי להיות ביום המאושר שלכם ולהעניק לכם את האלבום הדיגיטלי המלא מהחינה שלכם, 
...
<body>
    <div class="container">
        ...
        <div class="message">
            <p>שמחתי להיות ביום המאושר שלכם ולהעניק לכם את האלבום הדיגיטלי המלא מהחינה שלכם, 
    ...
    const finalResponse = new Response(stream, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="Henna-Album-Sapir-Idan-18-09-2025.zip"`,
