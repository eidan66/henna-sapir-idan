...
async function processRecord(record: S3EventRecord): Promise<void> {
  ...
  // Only process files in the raw folder - should be henna-sapir-idan/raw/
  if (!key.includes('/raw/')) {
  ...
  try {
    ...
    // Expected format: henna-sapir-idan/raw/{uuid}.{ext}
    const pathParts = key.split('/');
    const coupleId = pathParts[0]; // Should be "henna-sapir-idan"
